...
function skillDoc(name, description, body, extras = '') {
    return `---
name: ${name}
...
test('loadSkills discovers spec-compliant skill directories and parses metadata', async () => {
    ...
    await writeFile(path.join(homeDir, '.agents', 'skills', 'pdf-processing', 'SKILL.md'), skillDoc('pdf-processing', 'Extract text and tables from PDF files when the user asks for PDF work.', '# PDF Processing\nUse scripts/extract.py when appropriate.', 'license: Apache-2.0\ncompatibility: Requires Python 3\nmetadata:\n  author: example\n  version: "1.0"\nallowed-tools: Bash(python:*) Read\n'));
...
test('loadSkills ignores malformed or non-compliant skills', async () => {
    const cwd = await makeTempDir('protoagent-skills-cwd-');
    const homeDir = await makeTempDir('protoagent-skills-home-');
    await writeFile(path.join(homeDir, '.agents', 'skills', 'missing-description', 'SKILL.md'), '---\nname: missing-description\n---\n\nNo description here.\n');
    await writeFile(path.join(homeDir, '.agents', 'skills', 'Bad-Name', 'SKILL.md'), skillDoc('Bad-Name', 'Uppercase names are invalid.', 'Bad'));
    await writeFile(path.join(homeDir, '.agents', 'skills', 'wrong-dir', 'SKILL.md'), skillDoc('different-name', 'Directory mismatch.', 'Bad'));
    await writeFile(path.join(homeDir, '.agents', 'skills', 'broken-yaml', 'SKILL.md'), '---\nname: broken-yaml\ndescription: [unterminated\n---\n\nBroken\n');
    const skills = await loadSkills({ cwd, homeDir });
...
test('project-level skills override user-level skills with the same name', async () => {
    const cwd = await makeTempDir('protoagent-skills-cwd-');
    const homeDir = await makeTempDir('protoagent-skills-home-');
    await writeFile(path.join(homeDir, '.agents', 'skills', 'code-review', 'SKILL.md'), skillDoc('code-review', 'User skill description.', 'user body'));
    await writeFile(path.join(cwd, '.agents', 'skills', 'code-review', 'SKILL.md'), skillDoc('code-review', 'Project skill description.', 'project body'));
    const skills = await loadSkills({ cwd, homeDir });
    assert.equal(skills.length, 1);
...
test('buildSkillsCatalogSection only exposes metadata and locations', async () => {
    const cwd = await makeTempDir('protoagent-skills-cwd-');
    const homeDir = await makeTempDir('protoagent-skills-home-');
    await writeFile(path.join(cwd, '.agents', 'skills', 'data-analysis', 'SKILL.md'), skillDoc('data-analysis', 'Analyze datasets when users ask for charts or reports.', '# Internal instructions'));
    const skills = await loadSkills({ cwd, homeDir });
...
test('initializeSkillsSupport registers activate_skill with constrained enum and allowlists skill directories', async () => {
    const cwd = await makeTempDir('protoagent-skills-cwd-');
    const homeDir = await makeTempDir('protoagent-skills-home-');
    await writeFile(path.join(cwd, '.agents', 'skills', 'lint-fixes', 'SKILL.md'), skillDoc('lint-fixes', 'Fix lint errors when asked to clean up style issues.', '# Lint Fixes'));
    ...
    assert.deepEqual(activateTool.function.parameters.properties.name.enum, ['lint-fixes']);
...
test('activateSkill returns wrapped body and bundled resources without frontmatter', async () => {
    ...
    await writeFile(path.join(skillDir, 'SKILL.md'), skillDoc('pdf-processing', 'Use for PDF extraction tasks.', '# PDF Processing\nSee references/REFERENCE.md and scripts/extract.py.'));
...
test('generateSystemPrompt includes skill catalog and activation guidance, not eager-loaded instructions', async () => {
    ...
    const cwd = await makeTempDir('protoagent-skills-cwd-');
    const homeDir = await makeTempDir('protoagent-skills-home-');
    await writeFile(path.join(cwd, '.agents', 'skills', 'repo-docs', 'SKILL.md'), skillDoc('repo-docs', 'Use when documenting repository behavior.', '# Hidden body\nDetailed instructions'));
    process.chdir(cwd);
...
test('validatePath allows access to activated skill resources via allowlisted roots', async () => {
    ...
    await writeFile(path.join(skillDir, 'SKILL.md'), skillDoc('pdf-processing', 'Use for PDF extraction tasks.', '# Body'));
